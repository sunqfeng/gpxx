...
//		userinfo stUserinfo = new userinfo();
	...
	public ReMsg insert_userinfo(userinfo stUserinfo_in)
	{
		// TODO Auto-generated method stub

		String obj = stuserinfopub.scuserid();//生成userid

		stUserinfo_in.setUserid(obj); //生成userid
		stUserinfo_in.setRole("1"); //普通人员1
		stUserinfo_in.setLoginstatus("1"); //注册以后默认状态为登录1,0:销户状态

		Integer result = userinfodao.insert_userinfo(stUserinfo_in);
		if (result != 1 )
		{
			remsg.setCode("120");
	...
	public ReMsg check_name(String name)
	{
		// TODO Auto-generated method stub

		stUserinfo = userinfodao.sel_userinfo_username(name);
		if (stUserinfo.getUsername().trim().equals(name.trim()) ) //该用户已经存在
		{
			remsg.setCode("130");
			remsg.setMsg("该用户已经存在");
		}
